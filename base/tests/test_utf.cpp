...
TEST(Utf8, CountCharacters)
{
    // 1 + 1 + 1 + 1 = 4 codepoints
    EXPECT_EQ(Utf8::Count(u8"Ač€Z"), 4);
}

TEST(Utf8, ConversionFromUtf16)
{
...
TEST(Utf8, IdentityConversionFromUtf8)
{
...
TEST(Utf16, CountMixedString)
{
    EXPECT_EQ(Utf16::Count(u"A\u010D\U0001F600Z"), 4);
}

TEST(Utf16, ConversionFromUtf8)
{
...
TEST(Utf16, ConversionFromUtf32)
{
    std::u32string src = U"ABCč€";
...
TEST(Utf32, CountCharacters)
{
    EXPECT_EQ(Utf32::Count(U"ABCč€"), 5);
}

TEST(Utf32, ConversionFromUtf8)
{
    EXPECT_EQ(Utf32::FromUtf8(u8"ABCč€"), U"ABCč€");
}

TEST(Utf32, ConversionFromUtf16)
{
